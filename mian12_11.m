...
combined_index = [elements_index{1};elements_index{2};elements_index{3}; elements_index{4}];
...
excitation_freq_Hz=118;
excitation_full = [118,118]; %[Hz]； 102.6  % 4个不同的激励频率%102.6,102.6,102.6,102.6
...
for i = 1:size(elements,1)
    idx = elements(i,:); % [n1 n2 n3 n4 m12 m23 m34 m41]
    xy = nodes_ext(idx, 1:2);
    % 真实八节点壳单元边
...
for i = 1:size(elements,1)
    idx = elements(i, [1 2 3 4]); % 八节点单元的四个角点
    xy = nodes_ext(idx, 1:2);
    % 画四条边
