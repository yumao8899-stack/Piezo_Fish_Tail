function [ek,theta,xv2i,xv1i,xv3i,zmtemp,v3i,d,jtemp]=bottomshellek_new(~,~,bottom_layers,H_total,dyhm,jdzb,jdzb1,dybh)
% dyhm: 单元编号
% jdzb: 单元上层节点的物理坐标
% jdzb1: 单元下层节点的物理坐标
% dybh: en
...
layers = {}; z =-t/2;
for i = 1:size(bottom_layers,1)
    props = bottom_layers(i,:);
    Q0 = Q_gexiangyixing(props(1),props(2),props(3),props(4),props(5),props(6));
    theta = props(8);
    Q = transform_Q(Q0, theta);
    h = props(7);
    layers{end+1} = struct('Q', Q, 'h', h, 'z0', z, 'z1', z + h);
    z = z + h;
end
...
ek=zeros(40);eki=zeros(40);
for ii = 1:length(layers)
    d = layers{ii}.Q;
    z1 = layers{ii}.z1; z0 = layers{ii}.z0;
    bb = 2*z1/t; ab = 2*z0/t;
    for gaosii=1:8  % begin of Gauss integral
        ...
        djt=det(jtemp);
        tk=b'*d*b*djt;
        eki=eki+0.5*(bb-aa)*tk;
    end
    ek=ek+eki;eki=zeros(40);
end
end