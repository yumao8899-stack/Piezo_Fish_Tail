...
[nodes, elements, elements_index] = read_comsol('9.5yu.mphtxt');
...
combined_index = [elements_index{1}; elements_index{2}]; 
...
%% 模态叠加法 
% 定义激励 
excitation_freq_Hz = 150; %[Hz] ；150   113.47
...
figure(102)                   
plot(time_vector,dip(jth,1201:1601)*1e3)
xlabel('Time (seconds)')
ylabel('displacement(mm)')
hold on
xlim([min(time_vector), max(time_vector)]);
...
% 绘制位移最大的时刻的变形云图 
[~, max_disp_time_index] = max(abs(time_vector(jth, :)));
displacement_snapshot = time_vector(:, max_disp_time_index);
...
figure(101)
plot(f, P1, 'b-','LineWidth', 1.5);%20*log10(P1)
ylabel('Amplitude (dB)');
