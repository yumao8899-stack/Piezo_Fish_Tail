...
% ====== 观察 a1 对【系统谐振频率】的巨大影响 ======
f_range = 1:0.1:8; % 扫描激振频率范围 (Hz)
a1_test_values = [-2, 0, 2]; % 选取三个差异较大的 a1 值进行对比测试

figure; hold on; grid on;
colors = {'r', 'b', 'k'};

for i = 1:length(a1_test_values)
    a1_current = a1_test_values(i);
    amp_vs_freq = zeros(length(f_range), 1);
    
    fprintf('正在计算 a1 = %d 的频响曲线...\n', a1_current);
    
    for f_idx = 1:length(f_range)
        f_test = f_range(f_idx);
        ...
        for loopi = 1:size(elements, 1)
            ...
            % 这里的 a1 传入 a1_current
            [mw_eff, cw_eff] = wateradding(zmtemp_local, rho_fluid, mu, omega_test, a1_current, 1, 1);
        ...
        amp_vs_freq(f_idx) = abs(U(jth)) * 1000;
    end
    
    % 寻找当前 a1 下的谐振频率(峰值频率)
    [max_amp, peak_idx] = max(amp_vs_freq);
    peak_freq = f_range(peak_idx);
    fprintf(' -> a1 = %d 时，系统的谐振频率为: %.2f Hz\n', a1_current, peak_freq);
    
    plot(f_range, amp_vs_freq, 'Color', colors{i}, 'LineWidth', 2, ...
        'DisplayName', sprintf('a_1 = %d (Peak: %.2fHz)', a1_current, peak_freq));
end   

xlabel('Excitation Frequency (Hz)', 'FontSize', 12, 'FontWeight', 'bold');
ylabel('Tip Amplitude (mm)', 'FontSize', 12, 'FontWeight', 'bold');
title('Effect of a_1 on System Natural Frequency', 'FontSize', 14);
...
% 定义匿名目标函数 (固定其他已知参数)
obj_func = @(p) fit_fluid_params(p, f_exp, amp_exp, m, k, C_matrix, F_amp_vector, jth, elements, zmtemp_cell, rho_fluid, mu, disp_node);
